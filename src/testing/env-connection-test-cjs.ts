/**
 * Environment-based Azure DevOps Connection Test (CommonJS version)
 * 
 ...
 * 
 * Usage:
 * 1. npm run compile-test
 * 2. node dist-test/env-connection-test-cjs.js [project-name]
 * 
 * Example:
 * node dist-test/env-connection-test-cjs.js optional-project-name
 */

import * as dotenv from 'dotenv';
import * as path from 'path';
import { IPipelineDefinition, IPullRequest } from '../types/ado';
...
// Import as ESM since our project is configured with "type": "module"
...
async function runTest(): Promise<void> {
  ...
  try {
    ...
    (projectsResponse.value as IAzureDevOpsProject[]).forEach((project: IAzureDevOpsProject, index: number) => {
    ...
    if (projectName) {
      ...
      try {
        ...
        pipelineDefinitions.forEach((pipeline: IPipelineDefinition, index: number) => {
      ...
      try {
        ...
        if (!project) {
          ...
          if (pullRequests.length > 0) {
            ...
            pullRequests.slice(0, 5).forEach((pr: IPullRequest, index: number) => {
